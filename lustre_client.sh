...
if [[ -z ${JUMPBOX} ]]; then
    fatal "No JUMPBOX, can't proceed."
fi
...
log "NOW=$NOW  MGSIP=$MGSIP FILESYSTEMNAME=$FILESYSTEMNAME"

mount_nfs()
{
    yum -y install nfs-utils nfs-utils-lib
    echo "$JUMPBOX:$SHARE_HOME $SHARE_HOME    nfs4    rw,auto,_netdev 0 0" >> /etc/fstab
    mount -a
    mount
